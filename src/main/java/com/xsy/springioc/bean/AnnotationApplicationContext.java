package com.xsy.springioc.bean;
...
public class AnnotationApplicationContext implements ApplicationContext{
    ...
    private Map<Class,Object> beanFactory=new HashMap<>(); //存储Bean
    private static String rootPath;

    @Override
    public Object getBean(Class clazz) { //拿bean
        return beanFactory.get(clazz);
    }


    //根据给定的包的路径，我们去这个包中扫描标注了Instantiate的类，然后将对象的对象进行实例化
                ...
                if(child.isDirectory()){
                    ...
                    if(pathWithClass.contains(".class")){
                        ...
                        if(!clazz.isInterface()){
                            //判断这个类上是不是有@Be注解
                            Instantiate beAnnotation = clazz.getAnnotation(Instantiate.class);
                            if(beAnnotation!=null){
                                //如果有@Instantiate注解就是我们要进行实例化的对象了
        ...
        for (Map.Entry<Class, Object> entry : beanFactory.entrySet()) {
            ...
            if(fields.length!=0&&fields!=null){
                for (Field field : fields) {
                    //判断这个属性上有没有@Di注解
                    Devalue diAnnotation = field.getAnnotation(Devalue.class);
